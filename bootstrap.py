...
def is_linux_without_systemd():
    ...
    found = any(os.path.exists(path) for path in systemd_locations)
    found = found or shutil.which("systemd") is not None
    return not found
...
def install(unattended=False, devenv_version="v2.1.2", cachix_version="v1.11.1"):
    if has_nix():
        ...
        if not has_curl():
            ...
            sys.exit(2)
        extra_args = ""
        linux_without_systemd = is_linux_without_systemd()
        if linux_without_systemd:
            extra_args += " linux --init none"
        if unattended:
            extra_args += " --no-confirm"
        install_nix_cmd = (
    ...
    if not had_devenv:
        install_devenv_cmd = (
            ...
            f"--accept-flake-config github:cachix/devenv/{devenv_version}"
        )
    ...
    if has_cachix():
        if not has_correct_cachix_version(cachix_version):
            ...
            print("Skipping installation of cachix, already installed")
            had_cachix = True
    if not had_cachix:
        print("installing cachix")
        install_cachix_cmd = (
            f"{pathadd} && nix profile install "
            f"--accept-flake-config github:cachix/cachix/{cachix_version}"
        )
        run_or_exit(install_cachix_cmd)
    if is_nixos:
